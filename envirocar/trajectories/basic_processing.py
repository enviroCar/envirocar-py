...
class Processing():
    def __init__(self):
       print("Hello from basic processing!") # do we need anything here?

    def interpolate(self, points):
        """ Creates a trajectory from point data
        
        Keyword Arguments:
            GeoDataFrame {?} -- A GeoDataFrame containing the track points
        
        Returns:
            Something -- An interpolated trajectory, hopefully
        """
        ...
        input_coords_x = np.array(points_df_cleaned.lat.values.tolist())
        input_coords_y = np.array(points_df_cleaned.lng.values.tolist())
        ...
        tck, u = interpolate.splprep([input_coords_y, input_coords_x], s = 0)
