...
class Preprocessing():
    ...
    def remove_outliers(self, points, column):
        ...
        Keyword Arguments:
            ...
            column {String} -- columnn which to clean
        ...
        new_points = points.loc[(points[column] > fence_low) & (points[column] < fence_high)]
    ...
    def interpolate(self, points):
        ...
        def date_to_seconds(x):
            ...
            seconds = (date_time_obj-datetime.datetime(1970, 1, 1)).total_seconds()
            return int(seconds)
        ...
        input_time = np.vectorize(date_to_seconds)(np.array(points_df_cleaned.time.values.tolist()))

        # measurements TODO which else?
        input_co2 = np.array(points_df_cleaned['CO2.value'].values.tolist())
        input_speed = np.array(points_df_cleaned['Speed.value'].values.tolist())
        ...
        tck, u = interpolate.splprep([input_coords_x, input_coords_y, input_time, input_co2, input_speed], s = 0)
        step = np.linspace(0, 1, input_time[-1] - input_time[0])
        new_points = interpolate.splev(step, tck) # interpolating so many points to have a point for each second
        ...
        interpolated_df.columns = ['lng', 'lat', 'time_seconds', 'CO2.value', 'Speed.value']
        interpolated_df['time'] = np.vectorize(seconds_to_date)(interpolated_df['time_seconds'])
        interpolated_df.drop(['time_seconds'], axis=1) # TODO is there need for this column?
