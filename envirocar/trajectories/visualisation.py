...
class Visualiser():
    ...
    def aggregateByGrid(df,field,summary,gridSize):
        ...
        final_grid=final_grid[final_grid[summarized_field]>0].sort_values(by=summarized_field,ascending=False)
        final_grid[summarized_field]=round(final_grid[summarized_field],1)
        final_grid['x_centroid'],final_grid['y_centroid']=final_grid.geometry.centroid.x,final_grid.geometry.centroid.y
        return final_grid
    ...
    def plotAggregate(grid,field):
        ...
        folium.GeoJson(data_geojson,lambda feature: {'lineOpacity':0.4,
                                                          'color':'black',
                                                          'fillColor':None,
                                                          'weight': 0.5,
    ...
    def spatioTemporalAggregation(df,field,summary,gridSize):
        """
        Aggregates the given field on hour and weekday basis. Prepares data for mosaic plot
        ...
        Returns
        -------
        geodataframes: one each for larger grid and other for subgrids(for visualization purpose only)
            Aggregated grids with summary on it
        ...
        grouped = points_identified.groupby(['gridId','weekday','hour']).agg({field:[summary]})
        ...
        for currentGrid in unikGrid:
            ...
            xminn,xmaxx,yminn, ymaxx=xmin+(xmax-xmin)*0.05, xmax-(xmax-xmin)*0.05, ymin+(ymax-ymin)*0.05, ymax-(ymax-ymin)*0.05
            rowOffset=(ymaxx-yminn)/24.0
            colOffset=(xmaxx -xminn)/7.0
            for i in range(7):
                for j in range(24):
                    topy, bottomy,leftx,rightx=ymaxx-j*rowOffset,ymaxx-(j+1)*rowOffset,xminn+i*colOffset,xminn+(i+1)*colOffset
                    subgrid.append(Polygon([(leftx, topy), (rightx, topy), (rightx, bottomy), (leftx, bottomy)]))
                    mainGrid.append(currentGrid)
                    rowNum.append(j)
                    columnNum.append(i)
                    if len(grouped[(grouped['gridId_']==currentGrid) & (grouped['weekday_']==i) & (grouped['hour_']==j) ])!=0:
                        this_value=grouped[(grouped['gridId_']==currentGrid) & (grouped['weekday_']==i) & (grouped['hour_']==j) ].iloc[0][modified_fieldname]
        ...
        subgrid_gpd['gridId'] = subgrid_gpd.apply(lambda x:str(x['gridId'])+"_"+str(x['Weekday'])+"_"+str(x['hour']),axis=1)
    ...
    def MosaicPlot(mainGrid,grid,field):
        """
        Performs spatio temporal aggregation of data on weekday and hour, and prepares mosaicplot.

        Parameters
        ----------
        mainGrid :polygon geodataframe
            The grid geodataframe with grid and aggregated data in a column. Grid shoud have grid id or equivalent unique ids
        grid: Small subgrids, prepared for visualization purpose only represents an hour of a weekday
        field : string
            Fieldname with aggregated data
        ...
        lon=grid.geometry.centroid.x.mean()
        #Intialize a new folium map object
        m = folium.Map(location=[lat,lon],zoom_start=10,tiles='Stamen Toner')
        ...
        folium.GeoJson(mainGrid_geojson, lambda feature: {'lineOpacity':0.4,
                                                          'color':'#00ddbb',
                                                          'fillColor':None,
                                                          'weight': 2,
                                                          'fillOpacity':0}).add_to(m)
        ...
        folium.GeoJson(
           data_geojson,
            ...
            highlight_function=lambda feature:{'weight':3,'color':'black','fillOpacity':1},
            tooltip=folium.features.GeoJsonTooltip(fields=['Weekday','hour',field])).add_to(m)

       #format legend
        field=field.replace("_"," ")
        # add a legend
        colormap_rn.caption = '{value} per grid by weekday and hour'.format(value=field)
        colormap_rn.add_to(m)
    ...
    def OriginAndDestination(df):
        """
        Return dataframe for origin and destinations for tracks by their trackid
